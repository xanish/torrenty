...
func NewConnection(addr string) (Connection, error) {
	...
	if err != nil {
		return Connection{}, fmt.Errorf("error connecting to %s: %w", addr, err)
	}
...
func (c Connection) Handshake(infoHash, peerID [20]byte) error {
	...
	res, err := UnmarshalHandshake(c.conn)
	if err != nil {
		return fmt.Errorf("failed to unmarshal handshake response: %w", err)
	}

	if !bytes.Equal(res.InfoHash[:], infoHash[:]) {
		return fmt.Errorf("expected infohash to be %x, but got %x", infoHash, res.InfoHash)
	}
